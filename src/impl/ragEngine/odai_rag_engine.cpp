...
bool ODAIRagEngine::initialize(const RagConfig &config)
{
    if (!g_backendEngine->load_embedding_model(config.embeddingModelConfig))
    {
        ODAI_LOG(ODAI_LOG_ERROR, "Failed initialize RagEngine because failed to load Embedding Model");
        return false;
    }

    if (!g_backendEngine->load_language_model(config.llmModelConfig))
    {
...
int32_t ODAIRagEngine::generate_streaming_response(const string &query, const ScopeId &scopeId, odai_stream_resp_callback_fn callback, void *user_data)
{
    ...
    return g_backendEngine->generate_streaming_response(prompt, callback, user_data);
}
...
bool ODAIRagEngine::load_chat_session(const ChatId &chat_id)
{
    ...
    if (!g_db->get_chat_config(chat_id, chat_config))
    {
    ...
    if (!g_backendEngine->load_language_model(chat_config.llmModelConfig))
    {
    ...
    if (!g_backendEngine->load_chat_messages_into_context(chat_id, messages))
    {
                                                        ...
                                                        odai_stream_resp_callback_fn callback, void *user_data)
{
    ...
    int32_t total_tokens = g_backendEngine->generate_streaming_chat_response(chat_id, final_prompt, internal_callback, &buffer_ctx);
    ...
    // ToDo : Wrap the insert chat messages in a transaction

    // Save messages to database
    if (!g_db->insert_chat_messages(chat_id, messages_to_save))
    {
